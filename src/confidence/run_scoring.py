"""
Phase 2 Intern 1 — Pilot confidence scoring runner.

Loads existing CoT drafts from batch_results JSON (produced by Phase 1),
runs both scorers, and outputs per-step confidence scores for Intern 3.

Usage:
    python run_scoring.py --dataset gsm8k --n 5 --method both
    python run_scoring.py --dataset hotpotqa --n 5 --method logprob
"""
import argparse
import json
import logging
...
def load_batch_results(dataset: str) -> list:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"Run Phase 1 batch_runner first: python batch_runner.py --dataset {dataset}"
        )
    with open(path) as f:
        return json.load(f)
...
def run_scoring(dataset: str, n: int, method: str):
    ...
    for i, sample in enumerate(samples):
        ...
        if not steps:
            logger.warning(f"No steps found for sample {sample_id}, skipping.")
            continue
        ...
        # Annotate each step with confidence scores
        annotated_steps = []
        for step in steps:
            entry = {
            ...
            if method == "both" and isinstance(scores, dict):
                idx = step["step_id"] - 1
    ...
    if results:
        ...
        for r in results:
            for s in r["steps"]:
                if "confidence_logprob" in s and s["confidence_logprob"] is not None:
                    all_lp.append(s["confidence_logprob"])
                if "confidence_sampling" in s and s["confidence_sampling"] is not None:
                    all_samp.append(s["confidence_sampling"])
        if all_lp:
            logger.info(f"Logprob scores   — mean: {sum(all_lp)/len(all_lp):.4f}, min: {min(all_lp):.4f}, max: {max(all_lp):.4f}")
        if all_samp:
            logger.info(f"Sampling scores  — mean: {sum(all_samp)/len(all_samp):.4f}, min: {min(all_samp):.4f}, max: {max(all_samp):.4f}")
